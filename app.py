...
# 分頁邏輯直接用到的分析函式(快取包裝/輔助/常數一律來自 ui_common)
from indicators import enrich
from signals import scan_one
from backtest import backtest_signal, backtest_all, benchmark_return, BACKTESTABLE
from fundamentals import name_of
from score import rank_codes
...
with tab_cmp:
    ...
    if cmp_mode == "類股強弱":
        ...
        if secdf.empty:
            ...
            st.plotly_chart(hbar_sector(secdf, f"類股強弱 — 近{sdays}交易日平均報酬"),
...
with tab_market:
    ...
    if st.session_state.get("market_loaded"):
        ...
        if sec.empty:
            ...
            st.plotly_chart(hbar_sector(sec), width="stretch", config=PLOTLY_CONFIG)
